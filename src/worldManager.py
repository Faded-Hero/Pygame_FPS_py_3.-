...
class WorldManager(object):
    ...
    def draw(self, surface):
        ...
        if self.background is None:
            self.background = pygame.transform.scale(pygame.image.load("pics/background.png").convert(), (640,480))
        ...
        for sprite in self.sprite_positions:
            ...
            invDet = 1 / (self.camera.planex * self.camera.diry - self.camera.dirx * self.camera.planey) #required for correct matrix multiplication
          
            transformX = invDet * (self.camera.diry * spriteX -
                                   self.camera.dirx * spriteY)
            transformY = invDet * (-self.camera.planey * spriteX +
                                   self.camera.planex * spriteY) #this is actually the depth inside the surface, that what Z is in 3D
            ...
            drawEndY = int(spriteHeight / 2 + h / 2)
